...
# Requires: docker, jq, GH_DEL_TOKEN env var (classic PAT with delete:packages scope)
set -euo pipefail
...
# --provenance=false: don't push BuildKit attestation manifests.
# Without this, BuildKit pushes untagged "referrer" manifests alongside :latest
# which are then impossible to safely distinguish from old orphaned digests.
docker build \
  --provenance=false \
...
# Every untagged version is now a guaranteed orphan (old :latest displaced by new push).
cleanup_untagged() {
  local pkg="$1"
